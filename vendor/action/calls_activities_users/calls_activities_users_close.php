...
try
{
    ...
    $CallsActivitiesUsersValidate->setDateClose(date('Y.m.d h:m:s'));
    ...
    else
    {
        ...
        if ($CallsActivitiesUsers->SaveClose($CallsActivitiesUsersValidate->getCallActivityId(), $CallsActivitiesUsersValidate->getUserId(), $CallsActivitiesUsersValidate->getDateClose()))
        {
            ...
            $text = 'Encerrando participação: ' . '#'. $resultCallsActivities->call_activity_id . ' - ' . $resultCallsActivities->name;

            /** Publico uma mensagem */
            $CallsMessages->Save(0, $CallsActivitiesUsersValidate->getCallId(), $CallsActivitiesUsersValidate->getUserId(), $CallsActivitiesUsersValidate->getCompanyId(), $text, $CallsActivitiesUsersValidate->getDateClose(), json_encode($history, JSON_PRETTY_PRINT));
